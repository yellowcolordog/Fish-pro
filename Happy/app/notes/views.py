...
def like_views():
    ...
    user_id=session['id']
    # 查询帖子点赞踩表有没有raise数据
    r_old = Raise.query.filter_by(user_id=user_id,note_id=note_id).first()
    # 如果有数据,就删了
    if r_old:
...
def unlike_views():
    ...
    user_id=session['id']
    # 查询帖子点赞踩表有没有raise数据
    u_old = Down.query.filter_by(user_id=user_id,note_id=note_id).first()
    # 如果有数据,就删了
    if u_old:
    ...
    # 如果没有就添加
        u_new = Down()
        u_new.note_id=note_id
        u_new.user_id=user_id
