...
def friends2_views():
    if "id" in session and "name" in session:
        ...
        id = session['id']
        # 下面是通过id查关注表得到关注的关注人对象和粉丝对象
        user = Users.query.filter_by(id=id).first()
        fans = db.session.query(User_attention.user_fan_id).filter(User_attention.user_star_id == id).all()
        idos = db.session.query(User_attention.user_star_id).filter(User_attention.user_fan_id == id).all()
        fans_l=[]
        for f in fans:
            user = Users.query.filter_by(id=f[0]).first()
            fans_l.append(user)
        idos_l=[]
        for f in idos:
            user = Users.query.filter_by(id=f[0]).first()
            idos_l.append(user)
        # fans_l 和 ido_l 保存的这些人
        return render_template('friends2.html',params=locals())
    else:
        return render_template('friends.html')
    
@main.route('/friendtest')
def friendtest_views():
    user = Users.query.filter_by(id=3).first()
    print(user)
    return render_template('friends2.html',user=user)

@main.route('/get_notes')
def get_notes_views():
    # 没用id是因为session
    userId=int(request.args['userId'])
    get_notes = Notes.query.filter_by(user_id=userId).all()
    return ''
